...
type sessionTransportPath int

const (
	sessionTransportDTLS sessionTransportPath = iota
	sessionTransportDirectUDP
	sessionTransportRawTurnNoDTLS
)

func selectSessionTransportPath(tp *TurnParams) sessionTransportPath {
	if tp.NoTLS {
		return sessionTransportDirectUDP
	}
	if tp.RawMode {
		return sessionTransportRawTurnNoDTLS
	}
	return sessionTransportDTLS
}
...
func RunSession(
	...
	if useWrap && (tp.NoTLS || tp.RawMode) {
	...
	go func() {
		...
		for {
			...
			written, writeErr := activeConn.Write(pkt)
			if tp.RawMode {
				if writeErr != nil {
					d.rawStats.sendErrors.Add(1)
				} else {
					d.rawStats.rawSendPackets.Add(1)
					d.rawStats.rawSendBytes.Add(uint64(written))
				}
			}
			putPktBuf(pkt)
			if writeErr != nil {
	...
	go func() {
		...
		for {
			...
			if tp.RawMode {
				d.rawStats.rawRecvPackets.Add(1)
				d.rawStats.rawRecvBytes.Add(uint64(n))
			}
			...
			default:
				...
				putPktBuf(pkt)
				if tp.RawMode {
					d.rawStats.returnChDrops.Add(1)
				}
			}
		}
	}()
